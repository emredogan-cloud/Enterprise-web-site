# Kitap 05 — Epictetus: The Discourses and Enchiridion
### Tam rapor · Vâliçe Press · 2026-09-06 · Valice Classics #3

> Kök dizindeki Founder kopyası. Mühendislik kopyası
> `Valice-Press-Site/docs/execution/public-domain/BOOK-05-REPORT.md`.
> Dal: `feature/book-05-production` — **main'e birleştirilmedi.**

## Türkçe özet

**Kitap yarı yayımlanmıştı.** Faz 1 e-kitabı satışa koymuş ama kitabı hiç
bitirmemişti: `SPEC.md` her bölümünde *NOT STARTED*, 12 kapıdan 8'i başlamamış,
her format `not_planned`, arkasında yüklenebilir dosya olmayan bir "yakında"
ciltsiz, ve yol haritasının bu kitaba verdiği ticari gerekçe olan paket hiç yok.

**Kanonik başlık.** Yedi kaynak okundu; dördü aynı dizede birleşiyor ve e-kitap
Faz 1'den beri o adla satılıyor: `Epictetus: The Discourses and Enchiridion`.
Yol haritasının satırı kitap yazılmadan önceki bir *plan* başlığıydı. Canlı bir
ürün plana uysun diye yeniden adlandırılmaz; **kapak taşındı.** Ayrı olarak
`metadata.title` (KDP liste alanı) **(Annotated)** ekini taşır — ev kuralı, iki
lint zorunlu kılıyor; ikisini tek dizeye indirmeye çalıştım, lintler yakaladı.

**Kapak: üç kusur, hiçbiri sanat eseri yeniden çizilmeden düzeltildi.** İki
kapak da preflight'tan kalıyordu (gömülü olmayan Helvetica — Enigmatica'nın
gerçek KDP reddi — ve `untitled`/`anonymous` metadata). Kapak **"and the
ENCHIRIDION"** diyordu: altın italik satır renkten bulundu, kendi kelime
boşluğundan bölündü, zemini sütun sütun yeniden kuruldu ve **sanat eserinin
kendi `and` harfleri** geri yapıştırıldı — font eşleştirilmedi, satır dışında
**0 piksel** değişti. KDP'nin barkod kutusu künye bloğunu kesiyordu: F-029'un
kaydettiği üç *inpainting* denemesi tekrarlanmadı, bunun yerine üstteki
boşluklardaki pay harcanarak çelenk ve künye **blok hâlinde yukarı taşındı** —
basılı son PDF'te **kutuda 0 metin pikseli**.

**E-kitap:** `build_epub.py` **hiç kapak eklemiyordu**; içindeki `cover.jpg`
elle atılmış, manifest'te tanımsız (yani hiçbir okuyucu göstermiyor) ve eskiydi.
Artık tanımlı, sırtta ilk, güncel görselle aynı. EPUBCheck 0/0/0.

**Paket gerçek bir işlem.** Sepet zaten kitap başına bir satır açıyor ve her id
için ayrı hak veriyor; eksik olan tek şey düşük fiyattı. Paket, iki üye fiyatına
kısıtlanmış bir Paddle indirimi. Paddle'ın kendi fiyat motoruyla doğrulandı:
ayrı ayrı **$19.98**, paket **$14.99**.

---

## İKİNCİ DÜŞMAN İNCELEMESİ — "bu kitabın hazır olmadığını kanıtla"

Bağımsız inceleyicinin kararı **HAZIR DEĞİL**'di ve önemli olan her noktada
haklıydı. **5 blocker, 7 ciddi, 5 küçük, 9 temiz.** Hepsi düzeltildi.

**Alıcılara ulaşan kusur — ve sadece bu kitapta değil.**
`build-digital-editions.mjs` Ghostscript çalıştırıyor; `pdfwrite` her fontu
yeniden kuruyor ve **ToUnicode haritalarını düşürüyor**. Alıcının indirdiği
dosyada **845 ASCII-dışı karakter yok oluyordu** — her uzun tire, her kıvrık
tırnak, her `æ` ve `ē`:

> basılı: `proairesis — the will — as the place where all real work happens.`
> **satılan:** `proairesis the will as the place where all real work happens.`

Üstelik "sıkıştırılmış" dosya kaynağından **30 KB büyüktü**. Artık türetilmiş
dosya yerini hak etmek zorunda: metin kaybediyor veya küçülmüyorsa baskı iç
bloğu olduğu gibi geçiyor. Katalog genelinde **on kitapta** iç blok korunuyor —
traditional-games 6.578 karakter, Codex Mythologica 1.075 kaybediyordu. Bu
kitabın master'ı yeniden yüklendi; **diğer dokuz canlı kitap F-032**, çünkü
onları yeniden yüklemek alıcıların elindekini değiştirir ve bu bir karardır.

**Vitrin iki nesil eski kapağı gösteriyordu** — `front-v1`, yani "and **the**
ENCHIRIDION". Yeniden alındı.

**Aparat tabanı, var olmayan bir kitap üzerinden ölçülmüştü.** `measure.py`
hiç basılmayan 129 kelimelik bölüm blurb'ünü, başlık başına üç kelime varsayan
bir kuralın uydurduğu 41 kelimeyi ve dizilmemiş 1.015 kelimelik Long biyografik
notunu sayıyordu. Artık her bileşen **basılı iç bloğa karşı doğrulanıyor**.
Dürüst rakam **%20,18** ve duyarlılık yanında yayımlandı — en katı okuma
%19,46 ile **kalıyor**, kaydedildi ve kullanılmadı.

**Concordance iddiası dörtte üç oranında abartıyordu.** *"Marcus Aurelius'un
kanıtlanabilir şekilde okuduğu dört pasaj"* — dört VERIFIED satırın **üçü
George Long'un kendi 1862 girişine ve notlarına** bağlı, Marcus'un metnine
değil. Basılı concordance sayfasının kendi başlığı zaten doğruydu; mağazada,
ödemede, companion'da ve iç blokta üç yerde geçen cümle değildi. Hepsi
düzeltildi.

**RIGHTS.md ve KDP rehberi Founder'a kapağın tipografik olduğunu söylüyordu.**
Kapak, Founder'ın sağladığı **YZ ile üretilmiş** sanat eseri ve rehberin YZ
bölümü *görseller* için "generated" demiyordu bile — o rehberi izleyen Founder
KDP'nin zorunlu beyanını yanlış cevaplardı. Üçü de düzeltildi.

**Paketin belgelenmiş güvenlik ağı yoktu.** `restrict_to` bir yarıçaptır, ön
koşul değil: tek kitap + indirim kimliği canlı fiyat önizlemesinde $9.99 − $4.99
veriyor. Gerçek ön koşul sunucu tarafındaki `matchBundle` ve artık testi var.

**Düzeltmelerden sonra:** 393 test · eslint temiz · `tsc` temiz · build temiz ·
metadata/claim/rights/compliance lintleri **ok** · `selftest` ok ·
`validate-catalog` 80 geçti **0 hata** · iç blok **176 sayfa** preflight temiz,
QR okunuyor · alıcının PDF'i 845 karakterini geri aldı.

**Geriye tek iş kalıyor ve o Amazon hesabına ait:** ciltsiz baskının
yüklenmesi (`FOUNDER.md → F-031`).

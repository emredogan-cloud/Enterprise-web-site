  ...
  return (
    <aside className="space-y-5 lg:sticky lg:top-24 lg:self-start">
      ...
      >
        ...
        <p className="mb-3 px-3 pt-2 text-[12px] lg:text-[10px] font-semibold uppercase tracking-[0.3em] text-fg-soft">
          Account
        </p>
...
function SidebarItem({ item, active }: { item: NavItem; active: boolean }) {
  ...
  if (item.kind === "soon") {
    return (
      ...
      >
        {inner}
        <span className="rounded-full border border-white/[0.08] bg-white/[0.03] px-2 py-0.5 text-[12px] lg:text-[9px] font-semibold uppercase tracking-[0.18em] text-fg-soft">
          Soon
        </span>
      </button>
    );

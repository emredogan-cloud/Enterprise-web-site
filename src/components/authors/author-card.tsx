...
export function AuthorCard({ author }: { author: DemoAuthor }) {
  ...
  return (
    <Link
      ...
      className={`group block focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#33f0aa]/60 rounded-[22px] ${
        isFeatured ? "" : ""
      }`}
    >
      <article
        className={`home-card-hover home-glass relative overflow-hidden rounded-[22px] ${
          isFeatured
            ? "border-[#33f0aa]/70 shadow-[0_24px_56px_-20px_rgba(0,0,0,0.7),0_0_30px_-8px_rgba(51,240,170,0.4)]"
            : "border-white/[0.08]"
        }`}
      >
        {/* Portrait */}
        <div className="relative aspect-[3/4] w-full">
          ...
          {isFeatured && (
            <span
              className="absolute right-2.5 top-2.5 rounded-full px-2 py-0.5 text-[9px] font-bold uppercase tracking-[0.15em]"
              style={{
        ...
        <div className="p-4 sm:p-5">
          {/* Name */}
          <h3 className="font-serif text-[16px] font-medium leading-tight text-[#e6e6e0] transition-colors group-hover:text-[#33f0aa] sm:text-[17px]">
            {author.name}
          </h3>

          {/* Role */}
          <p className="mt-1 text-xs text-[#88918a]">{author.role}</p>

          {/* Signature works */}
          <p className="mt-3 line-clamp-1 text-xs text-[#a7a7a0]">
            {author.works}
          </p>
          ...
          <div className="mt-4 flex items-center justify-between">
            ...
            <span
              aria-hidden
              className="flex h-8 w-8 items-center justify-center rounded-full border border-white/[0.1] bg-white/[0.03] text-[#a7a7a0] transition-all group-hover:border-[#33f0aa]/50 group-hover:bg-[#33f0aa]/10 group-hover:text-[#33f0aa] group-hover:shadow-[0_0_14px_rgba(51,240,170,0.4)]"
            >

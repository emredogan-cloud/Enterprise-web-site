...
import type { PortraitTheme } from "@/components/authors/demo-authors";
...
export default async function AuthorPage({
  ...
  const { head, tail } = splitNameForAccent(author.name);
        ...
        <CinematicHero
          ...
          panel={
            <div className="group h-full w-full">
              <AuthorPortrait theme={DEFAULT_PORTRAIT} />
            </div>
          }
        />
            ...
            <h2 className="mt-5 font-serif text-[28px] font-medium leading-tight text-[#e6e6e0] sm:text-[36px]">
              {author.books.length === 0
                ...
                : author.books.length === 1
                  ? "One published title"
                  : `${author.books.length} published titles`}
            </h2>
          </header>
